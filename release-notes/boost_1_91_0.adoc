...
** Added built-in support for **Sentinel** deployments. Sentinel can be configured
   by using `config::sentinel`. The library takes care of master/replica discovery,
   error detection and reconnection.
   PR 345, PR 391, issue 237
** Added **generic_flat_response**. High performance flat_response replacement.
   Contiguous memory, zero allocations in steady state. PR 340, PR 356, PR 358, PR 378, PR 383, issue 263
   Thanks Nikolai Vladimirov.
...
*** Added support for subscription tracking, which restores
    ...
    request::{subscribe, psubscribe, unsubscribe, punsubscribe}.
    Check the examples. PR 375, issue 367.
*** Added push_parser, a view that facilitates parsing
    PubSub-related messages. See examples.
*** **connection::{async_receive, receive}** are now **deprecated**. Prefer using
    async_receive2, which allows for faster batch processing.
    PR 381, PR 340, issue 331.
*** **consume_one** is now **deprecated**. Prefer using generic_flat_response::clear,
    instead. Check the pubsub examples for more info.
*** Improved the documentation and examples.
** **config::{username, password, client_name, database_index}**
   are now **deprecated**. Please use config::setup, instead.
   Added request::{hello, hello_setname} to make composing
   setup requests easier. PR 392.
** Log message levels have been adjusted to be more relevant.
   level::error is used for configuration and network errors,
   level::info for successful connection attempts,
   and level::debug for verbose output. PR 389, issue 395
...
** Fixed a problem causing request::push_range to generate invalid commands
   when passed a range with std::tuple elements. PR 363, issue 360
