...
int main() {
    ...
    // std::cin >> server_ip leaves the newline character generated
    // by pressing Enter in the input stream.
    //
...
//
// This means the same executable can act as either the connecting
// peer or the listening peer.
...
int main() {
    ...
    // std::cin >> local_port left the newline produced by pressing
    // Enter in the input stream.
    //
...
void handle_client(tcp::socket socket) {
    try {
        ...
        // for requests larger than this buffer and for partial
        // messages.
        char data[1024];
...
void websocket_session(tcp::socket socket) {
    try {
        ...
        while (true) {
            ...
            // a text message; if it sent binary data, the response
            // remains binary.
            ws.text(ws.got_text());
