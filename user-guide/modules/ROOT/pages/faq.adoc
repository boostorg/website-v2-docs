...
* boost:mp11[] and boost:hana[] both work with metaprogramming, often with overlapping spaces, but their usage patterns can conflict. MP11 uses a more classic, compile-time only, and more explicit metaprogramming model, while Hana includes both compile-time and run-time metaprogramming functions, which introduce ambiguity when mixing the two libraries. Best to choose one of these libraries, unless you can ensure clean separation between the two.
...
+
- [ ] Audit *transitive dependencies* with tools like `bcp` or `boostdep`.
...
+
- [ ] Encapsulate low-level or unsafe Boost operations behind clean APIs.
