...
bool parseSentence(const std::string& input) {
    ...
    // Define the basic grammar elements. 'word' matches one or more alphabetic
    // characters, while the other rules give those words grammatical meaning.
    ...
    // Combine the individual parsers into the complete grammar:
    // "The" followed by an adjective, noun, and verb, ending with a period.
    // '>>' means that each parser must match in the specified sequence.
    qi::rule<std::string::const_iterator, std::string()> sentence =
        article >> qi::lit(' ') >> adjective >> qi::lit(' ') >> noun >> qi::lit(' ') >> verb >> qi::lit('.');

    // Spirit advances 'begin' as characters are successfully consumed, so
    // begin == end afterwards confirms that the entire input was parsed.
...
int main() {
    ...
    // Read and validate sentences repeatedly until the user enters "exit".
    std::string input = "";
    while (input != "exit")
    {
        ...
        // parseSentence() returns true only when the input conforms completely
...
// Populate a qi::symbols parser from a list of words. Each word is stored
// as both the recognized symbol and its associated string value.
...
int main() {
    ...
    // Define the vocabulary understood by our grammar. A word must appear in
    // the appropriate category to be recognized as that part of speech.
    ...
    // qi::symbols represents a set of recognized words, making it useful for
    // vocabulary-based parsing such as nouns, verbs, adjectives, and so on.
    ...
    // Read sentences repeatedly until the user enters "exit".
    std::string input = "";
    ...
    while (input != "exit")
    {
        ...
        if (input != "exit")
        {
            // Spirit advances 'begin' as it consumes input; checking begin == end
            // after parsing therefore verifies that the entire sentence was used.
            auto begin = input.begin();
            auto end = input.end();

            // Variables populated by the grammar's semantic actions.
            std::string det1, adj1, noun1, adv, verb, adj2, noun2;

            // Grammar: Determiner [adjective] noun [adverb] verb [adjective] noun.
            // '>>' means sequence, '-' makes a parser optional, and qi::lit('.')
            // requires a literal period at the end of the sentence.
            bool success = qi::phrase_parse(
                ...
                // The skipper tells Spirit to ignore whitespace between grammar
                // elements, so spaces need not be explicitly included in the grammar.
                ascii::space
            );

            // phrase_parse() must succeed AND consume the entire input; otherwise
            // a valid prefix followed by unexpected text would still be accepted.
